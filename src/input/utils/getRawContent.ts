...
export const getRawContent = async <D = any>(document: D, options: TOptions<D>) => {
    const baseUrl = (() => {
    ...
    if (!langCulture.includes('-')) langCulture = LangCultureMap?.[langCulture.toLowerCase() as keyof typeof LangCultureMap] ?? 'en-US';
